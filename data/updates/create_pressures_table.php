...
class CreatePressuresTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('cnesmeteo_data_pressures', function($table)
        {
            $table->engine = 'InnoDB';

            $table->integer('measurement_id')->unsigned()->index(); // Measurement
            $table->decimal('value', 14, 6)->nullable(); // Value
        });

        // Foreign keys
        Schema::table('cnesmeteo_data_pressures', function($table)
        {
    ...
    {
        Schema::drop('cnesmeteo_data_pressures');
    }
